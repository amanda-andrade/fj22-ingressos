...
public class GerenciadorDeSessaoTest {
	...
	public void setup() {
		...
		gerenciador = new GerenciadorDeSessao(sessoes);
		filme = new Filme("Rogue One", Duration.ofMinutes(120), "Sci-fi", BigDecimal.TEN);
		sala = new Sala("Sci-fi", BigDecimal.TEN);		
		horario = LocalTime.parse("10:00:00");
	}

	@Test
	public void deveAdicionarUmaSessaoSeAListaEstiverVazia(){

		Sessao sessao = new Sessao(horario, filme, sala);
		
		boolean respostaObtida = gerenciador.cabe(sessao);
		
		Assert.assertTrue(respostaObtida);
	}
	
	@Test
	public void umaSessaoDeveTerminarNoMesmoDia(){
		horario = LocalTime.parse("23:00:00");
		
		Sessao sessao = new Sessao(horario, filme, sala);
		
		boolean respostaObtida = gerenciador.cabe(sessao);
		
		Assert.assertFalse(respostaObtida);
	}
